"""SQLite giriş qatı. Bütün sorğular bu modul üzərindən keçir.

Qeyd: layihə kiçik miqyaslı olduğu üçün bilərəkdən ORM istifadə olunmur -
sadə, oxunaqlı, parametrləşdirilmiş SQL kifayətdir.
"""
import sqlite3
import os
from werkzeug.security import generate_password_hash, check_password_hash

# create_app() tərəfindən init_app(app) ilə təyin olunur.
# Test zamanı hər testin öz müvəqqəti fayl yoluna işarə edə bilməsi üçün modul
# səviyyəsində saxlanılır (bax: tests/conftest.py).
...
def count_donors(blood_type=None, city=None, is_available=None):
    conn = get_db_connection()
    clause, params = _donor_filters_clause(blood_type, city, is_available)
    total = conn.execute("SELECT COUNT(*) FROM users" + clause, params).fetchone()[0]
    conn.close()
    return total
...
def get_blood_request_by_id(request_id):
    conn = get_db_connection()
    req = conn.execute(
        'SELECT br.*, u.full_name as author_name FROM blood_requests br JOIN users u ON br.user_id = u.id WHERE br.id = ?',
        (request_id,)
    ).fetchone()
    conn.close()
...
def create_donation_offer(request_id, donor_id, message=None):
    ...
    cursor = conn.cursor()
    cursor.execute(
        'INSERT INTO donation_offers (request_id, donor_id, message) VALUES (?, ?, ?)',
        (request_id, donor_id, message)
    )
    conn.commit()
...
def get_platform_stats():
    ...
    active_donors = conn.execute("SELECT COUNT(*) FROM users WHERE role = 'donor' AND is_available = 1").fetchone()[0]
    active_requests = conn.execute("SELECT COUNT(*) FROM blood_requests WHERE status = 'active'").fetchone()[0]
    fulfilled_requests = conn.execute("SELECT COUNT(*) FROM blood_requests WHERE status = 'fulfilled'").fetchone()[0]
    total_cities = conn.execute("SELECT COUNT(DISTINCT city) FROM users WHERE city IS NOT NULL AND city != ''").fetchone()[0]
    conn.close()
    return {
